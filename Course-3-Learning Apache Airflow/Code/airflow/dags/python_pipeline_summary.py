...
def read_csv_file():
    df = pd.read_csv('/home/adrahat/airflow/datasets/insurance.csv')
...
def groupby_smoker(ti):
    ...
    smoker_df.to_csv('/home/adrahat/airflow/output/smoker_stat.csv', index = False)
...
def groupby_region(ti):
    ...
    region_df.to_csv('/home/adrahat/airflow/output/region_stat.csv', index = False) 
	

with DAG(
        dag_id='python_summary_pipeline',
        description='Running a Python Pipeline',
        default_args=default_args,
        start_date=days_ago(1),
        schedule_interval='@once',
        tags=['python', 'transform', 'pipeline', 'summary']
) as dag:
